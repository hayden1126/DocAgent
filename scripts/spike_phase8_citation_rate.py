#!/usr/bin/env python3
"""Phase 8 spike: measure citation-emission rate on the LiteLLM backend.

Runs the `LiteLLMBackend` against `tests/golden/fixtures/tinylib_ts/` with a
README-style prompt, then measures what fraction of the generated
`<!-- ground: path:lines -->` citations resolve via DocAgent's existing
citations gate.

Output goes to stderr (one human-readable summary) and to a JSON file at
`.planning/decisions/0001-spike-results.json` for ADR-0001 ingestion.

Usage:
    # Ollama (local; requires `ollama pull llama3.1:8b` + daemon running):
    python scripts/spike_phase8_citation_rate.py --model ollama_chat/llama3.1:8b

    # Gemini (requires GEMINI_API_KEY env var):
    python scripts/spike_phase8_citation_rate.py --model gemini/gemini-2.5-flash

    # OpenRouter (requires OPENROUTER_API_KEY):
    python scripts/spike_phase8_citation_rate.py --model openrouter/anthropic/claude-sonnet-4-6

Citation gating definition (from ADR-0001):
- ≥80%: Ollama ships in BACKEND-01 as-is.
- 60-79%: Ollama ships with [unsupported-model] WARN; opt-in.
- <60%: Drop Ollama from BACKEND-01 scope.
"""
